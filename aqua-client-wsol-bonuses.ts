...
const SWAP_QUOTE_TOKENS = 0.2;    // WSOL tokens user pays
const BONUS_PERCENT = 10_000_000_000n; // 10% scaled by 1e9; denom 100_000_000_000
...
async function main() {
  ...
  const baseMint = await generateKeyPairSigner();
  {
    ...
    const { value: bh } = await rpc.getLatestBlockhash().send();
    const ixs = [
      getCreateAccountInstruction({ payer, newAccount: baseMint, lamports: rent, space, programAddress: TOKEN_PROGRAM_ADDRESS }),
      getInitializeMintInstruction({ mint: baseMint.address, decimals: DECIMALS, mintAuthority: payer.address }),
    ];
    await pipe(
      createTransactionMessage({ version: 0 }),
      (tx) => setTransactionMessageFeePayerSigner(payer, tx),
  ...
  {
    const { value: bh } = await rpc.getLatestBlockhash().send();
    const ix1 = await getCreateAssociatedTokenIdempotentInstructionAsync({ mint: baseMint.address, payer, owner: swapPda });
    const ix2 = await getCreateAssociatedTokenIdempotentInstructionAsync({ mint: WSOL_MINT, payer, owner: quoteVaultOwner.address });
    const mintBaseToVaultIx = getMintToInstruction({ mint: baseMint.address, token: baseVaultAta, amount: BigInt(MINT_BASE_AMOUNT) * BigInt(10 ** DECIMALS), mintAuthority: payer });
    await pipe(
      createTransactionMessage({ version: 0 }),
      (tx) => setTransactionMessageFeePayerSigner(payer, tx),
      (tx) => setTransactionMessageLifetimeUsingBlockhash(bh, tx),
      (tx) => appendTransactionMessageInstructions([ix1, ix2, mintBaseToVaultIx], tx),
      async (tx) => {
        const signed = await signTransactionMessageWithSigners(tx);
        await sendAndConfirmTransaction(signed as any, { commitment: 'confirmed' });
        return getSignatureFromTransaction(signed);
  ...
  console.log('✅ Vaults prepared; base funded; WSOL ATA for quote vault owner exists');
